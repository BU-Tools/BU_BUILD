...
proc AXI_IP_IRQ_SIMPLE {params} {
    ...
    #to trick Vivado into there being a IRQ_CTRL like thing, we make two not objects
    create_bd_cell -type ip -vlnv [get_ipdefs -filter {NAME == util_vector_logic}] $device_name
    set_property -dict [list CONFIG.C_SIZE {1} CONFIG.C_OPERATION {not} CONFIG.LOGO_FILE {data/sym_notgate.png}] [get_bd_cells $device_name]
    create_bd_cell -type ip -vlnv [get_ipdefs -filter {NAME == util_vector_logic}] ${device_name}_hack
    set_property -dict [list CONFIG.C_SIZE {1} CONFIG.C_OPERATION {not} CONFIG.LOGO_FILE {data/sym_notgate.png}] [get_bd_cells ${device_name}_hack]
    #connect up the two nots
    connect_bd_net [get_bd_pins ${device_name}/Op1] [get_bd_pins ${device_name}_hack/Res]
    ...
    connect_bd_net [get_bd_pins ${device_name}/Res] [get_bd_pins ${irq_dest}]
    set IRQ_CONCAT ${device_name}_IRQ
    create_bd_cell -type ip -vlnv  [get_ipdefs -filter {NAME == xlconcat}] ${IRQ_CONCAT}
    connect_bd_net [get_bd_pins ${IRQ_CONCAT}/dout] [get_bd_pins ${device_name}_hack/Op1]
