...
proc BUILD_AXI_ADDR_TABLE {device_name} {
    global axi_memory_mappings_addr
    global axi_memory_mappings_range
    set addr_seg [get_bd_addr_segs -quiet *SEG*${device_name}_*]
    puts ${addr_seg}
    if {[string length ${addr_seg}] >0} {
	set addr [format %X [lindex [get_property -quiet OFFSET ${addr_seg}] 0] ]
	set addr_range [format %X [lindex [get_property -quiet RANGE ${addr_seg}] 0] ]
	puts "  ${device_name}:  ${addr}:${addr_range}"
	dict set axi_memory_mappings_addr  ${device_name} ${addr}      
	dict set axi_memory_mappings_range ${device_name} ${addr_range}
    }
}
